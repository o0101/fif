...
// main function
  async function runAllTests() {
    ...
    console.log(`\n\n${greenCheck} ${totalPassed} tests passed!\n\n`);
...
// tests
  ...
  async function testHardenedBuffer() {
    console.log('Testing HBUFFER Type');
    ...
    const readBuf = handler.hbuffer().last.value;

    assertBufferEqual(buf, readBuf, 'HBUFFER');
...
// cryptographic signing
  function saveKeys() {
    privateKey = eddsa.utils.randomPrivateKey();
    publicKey = eddsa.getPublicKey(privateKey);
...
// process error catchers
  ...
  process.on('unhandledRejection', err => {
    console.log('\n\n');
    console.error(`Err test failure`, err);
    console.log('\n\n');
    process.exit(1);
  });
