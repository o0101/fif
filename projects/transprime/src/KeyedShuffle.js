...
export class KeyedShuffle {
  ...
  async shuffle(matrix, key) {
    const rows = matrix.length;
    if (rows == 0) return matrix;
    const cols = matrix[0].length;
    if (rows*cols > this.constructor.MAX_RANGE) {
      throw new Error(`Matrix cells must be ${this.constructor.MAX_RANGE} or smaller.`);
    }
    const flatArray = this.flatten(matrix);
    const keystream = this.getKeystream(key, flatArray.length);

    for (let i = flatArray.length - 1; i > 0; i--) {
      const j = await this.getRandomIndex(keystream, i + 1);
  ...
  async getRandomIndex(keystream, range) {
